...
Requires **JDK 11+** to compile the plugin. Gradle 9.6 (included in this repo) can run on **Java 17 or newer** (e.g. Java 26); on the first build it will **automatically download JDK 11** for compilation via the Gradle toolchain.

If auto-download fails, install [Eclipse Temurin 11](https://adoptium.net/temurin/releases/?version=11) and run again — Gradle will detect it.
...
| Build fails: `Cannot find a Java installation… languageVersion=11` | Use the included `gradlew` (Gradle 9.6). First run downloads JDK 11 automatically. Needs network access. Or install [Temurin 11](https://adoptium.net/temurin/releases/?version=11) manually. |
| Build fails on Java 26+ | Normal — Gradle 9.6 runs on Java 26; compilation uses auto-provisioned JDK 11. Do not downgrade to Gradle 8.x unless you also use JDK 11 to run Gradle. |
